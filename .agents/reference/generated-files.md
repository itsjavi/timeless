...
| `src/define/<tag>.ts`                                | One `define<Name>Element()` entrypoint per custom element. No side effect — importing it registers nothing                                                    |
| `src/register.ts`                                    | The aggregate side-effect entrypoint: registers every element as it is imported                                                                               |
| `src/register/<tag>.ts`                              | One side-effect entrypoint per custom element. `src/register/browser.ts` is hand-written and holds the server guard                                           |
