...
Every milestone file ends with a horizontal rule and an attribution line naming what model (detected
by the agent itself) actually generated the document, including reasoning effort when the agent
exposes one. For example:

```markdown
---

Generated by MODEL_ID (REASONING_EFFORT)
```

example: `Generated by Codex GPT-5.6 Sol (High)`

`RESULTS.md` additionally gets, once implementation has happened, e.g.:

```markdown
Implemented by MODEL_ID (REASONING_EFFORT)
```

example: `Implemented by CLaude Fable 5 (Max)`
...
- Milestones 001 to 018 predate the convention entirely, and 019 and 020 adopted it on `RESULTS.md`
  only. All of those stay as they are — do not backfill them. From 021 on, every file carries its
  footer, so a missing one there is an omission rather than history.
