...
CSS, class, and `define/*` imports are safe during server rendering. Keep `register/*` imports in
code the browser loads, because that is where they call `customElements.define` — on the server they
are inert, so a server-only import registers nothing.
