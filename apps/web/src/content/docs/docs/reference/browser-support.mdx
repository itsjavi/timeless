...
| `:state()` custom states | 125           | 17.4   | 126     | Toast, Color Picker, and Sheet drag state                 |
...
[Context Menu](/docs/components/context-menu/) is the single exception, and it is a platform gap
rather than a support one: there is no declarative way to open a surface at pointer coordinates, so
with scripting off the browser shows its own context menu and the authored `ui-menu` stays hidden.
Nothing reachable only from a context menu is reachable at all, in any browser, until the bundle
loads. [Sheet](/docs/components/sheet/)'s swipe-to-dismiss is a gentler version of the same shape: a
