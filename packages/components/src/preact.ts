/**
 * Preact declarations for every Timeless custom element, generated from the component
 * registry. Types only: importing this module adds no runtime code and no Preact dependency.
 *
 * Preact registers a listener for any `on*` prop, so `onui-change` binds the `ui-change` event with no wrapper.
 */
...
// @ts-ignore Preact is an optional consumer dependency.
